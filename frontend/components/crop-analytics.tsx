...
export default function CropAnalytics({
  ...
  useEffect(() => {
    async function fetchAnalytics() {
      if (!selectedCrop || !stateName || !districtName) return
      ...
      try {
        ...
        console.log("Analytics yield:", data)

        setAnalyticsData({
          yield: data.yield || data.predicted_yield,
          production: data.production || data.predicted_production,
        })
      } catch (err) {
    ...
    fetchAnalytics()
  }, [selectedCrop, stateName, districtName, season, landArea])
    ...
    <Card>
      ...
      <CardContent>
        {analyticsData ? (
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={[
              { name: "Yield (tons/ha)", value: analyticsData.yield },
              { name: "Production (tons)", value: analyticsData.production },
            ]}>
              <XAxis dataKey="name" />
              <YAxis />
              <Tooltip />
          ...
          <p className="text-muted-foreground">Select crop and location to view analytics</p>
        )}
      </CardContent>
    </Card>
  )
